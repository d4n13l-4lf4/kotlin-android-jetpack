...
import com.example.playpennydrop.data.PennyDropDatabase
import com.example.playpennydrop.data.PennyDropRepository
...
class RankingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        playerSummaries = Transformations.map(this.repository.getCompletedGameStatusesWithPlayers()) {
            statusesWithPlayers ->
            statusesWithPlayers
                .groupBy { it.player }
                .map { (player, statuses) ->
